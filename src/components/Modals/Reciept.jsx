...
const RecieptModal = ({ open, setOpen, OrderData }) => {
  const pdfRef = useRef();
      ...
      <div className="flex flex-col px-5 w-[750px]" ref={pdfRef}>
        ...
        <div className="flex justify-between items-center pt-3">
          ...
          <div>
            <div className="font-bold text-xl text-black">
              Receipt No {OrderData.orderNumber}
            </div>
            <div className="text-[#B5B5B5] text-lg">
              {moment(new Date(OrderData.createdAt * 1000)).format(
                "DD/MM/YYYY"
              )}
            </div>
            <div className="text-[#B5B5B5] text-lg">
              {moment(new Date(OrderData.deliveryTime)).format("HH:mm A")}
            </div>
          </div>
        </div>
        <div className="flex flex-col justify-start items-start mb-4">
          ...
          <div className="flex gap-x-1 text-[#8B8A8A]">
            ...
            <div className="">{OrderData.shop_name}</div>
          </div>
          <div className="flex gap-x-1 text-[#8B8A8A]">
            <div className="font-bold">Order No: </div>
            <div className="">{OrderData.orderNumber}</div>
          </div>
        </div>
        <div className="border-2 border-[#d9d9d9] rounded-lg">
          <ReceiptTable Data={OrderData.items} />
        </div>
