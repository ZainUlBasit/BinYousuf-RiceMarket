...
import CategoryItemDataCard from "./CategoryItemDataCard";
...
const StyledContainer = styled(motion.div)`
  ...
  width: 100%;
  margin: 1rem 0rem;
  border: 1px solid #f8c21f; /* Extra small devices (phones) */
  border-radius: 30px;
  padding: 10px 20px;
...
const CategoryItemCard = ({ data, children }) => {
  const container = {
  ...
  return (
    <StyledContainer variants={container} initial="hidden" animate="visible">
      <CategoryItemDataCard data={data} />
      <motion.div
        variants={item}
